-- Задача 2: Подчиненные Ивана Иванова с подсчетом подчиненных и задач (MySQL)
...
EmployeeTasks AS (
    SELECT 
        AssignedTo,
        COUNT(*) AS task_count,
        GROUP_CONCAT(TaskName ORDER BY TaskName SEPARATOR ', ') AS task_list
    FROM Tasks
    GROUP BY AssignedTo
),
EmployeeProjects AS (
    SELECT 
        e.EmployeeID,
        GROUP_CONCAT(DISTINCT p.ProjectName ORDER BY p.ProjectName SEPARATOR ', ') AS project_list
    FROM Employees e
